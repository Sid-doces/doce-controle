...
import { 
  ...
  ArrowRight, X, Percent, Share2, Download, Link2, Globe, Database, ShieldAlert, CloudLightning, ExternalLink, RefreshCw, Copy, Check
} from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ state, setState, daysRemaining }) => {
  ...
  const copyInviteLink = () => {
    ...
    const inviteBase64 = btoa(state.user.googleSheetUrl);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <header>
        ...
        <p className="text-gray-500 font-medium italic">Sua central de gestão.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          ...
          {isOwner && (
            <div className="bg-indigo-600 p-8 rounded-[40px] text-white shadow-xl shadow-indigo-100 space-y-4">
              <div className="flex items-center gap-3">
                 <Users size={24} />
                 <h3 className="font-black text-sm uppercase tracking-widest">Acesso da Equipe</h3>
              </div>
              <p className="text-[10px] font-bold opacity-80 leading-relaxed">
                Envie este link para seus funcionários. O app se configurará automaticamente no celular deles.
              </p>
              <button 
                onClick={copyInviteLink}
                className="w-full flex items-center justify-center gap-2 py-4 bg-white text-indigo-600 rounded-2xl font-black text-xs uppercase transition-all active:scale-95"
              >
                {copiedInvite ? <Check size={18} /> : <Copy size={18} />}
                {copiedInvite ? "Link Copiado!" : "Copiar Link de Convite"}
              </button>
            </div>
          )}
          ...
          <div className="bg-white p-8 rounded-[40px] border border-indigo-50 shadow-sm space-y-4">
            <div className="flex items-center gap-3 text-indigo-600">
               ...
               <h3 className="font-black text-sm uppercase tracking-widest">Planilha Cloud</h3>
            </div>
            <div className="space-y-4">
               <textarea 
                 placeholder="Cole a URL do Google Script aqui..."
               ...
               <div className="flex gap-2">
                  <button onClick={handleUpdateSheetUrl} className="flex-1 py-3 bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase shadow-sm">Conectar</button>
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white p-10 rounded-[45px] border border-gray-100 shadow-sm">
            <h3 className="text-lg font-black text-gray-800 mb-8 flex items-center gap-2"><Lock size={20} className="text-pink-500" /> Alterar Senha</h3>
          ...
          {isOwner && (
            <section className="bg-white p-10 rounded-[45px] border border-gray-100 shadow-sm">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-lg font-black text-gray-800 flex items-center gap-2"><Users size={20} className="text-pink-500" /> Equipe Autorizada</h3>
              ...
              <div className="space-y-3">
                {(state.collaborators || []).map(collab => (
                  <div key={collab.id} className="flex items-center justify-between p-5 bg-gray-50/50 rounded-[28px] border border-transparent">
                    ...
                    <button onClick={() => { if(confirm("Excluir?")) { const u = JSON.parse(localStorage.getItem('doce_users') || '{}'); delete u[collab.email]; localStorage.setItem('doce_users', JSON.stringify(u)); setState(prev => ({...prev, collaborators: prev.collaborators.filter(c => c.id !== collab.id)})); } }} className="text-gray-200 hover:text-red-500 p-2"><Trash2 size={18} /></button>
                  </div>
                ))}
              </div>
            </section>
        ...
        <div className="fixed inset-0 bg-pink-950/40 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <form onSubmit={handleAddCollaborator} className="bg-white w-full max-w-md p-10 rounded-[45px] shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="space-y-6">
              <input type="email" required placeholder="E-mail do Funcionário" className="w-full px-6 py-4 rounded-2xl border-2 border-gray-50 bg-gray-50 focus:bg-white focus:border-pink-500 outline-none font-bold" value={collabEmail} onChange={e => setCollabEmail(e.target.value)} />
              <input type="text" required placeholder="Senha de Acesso" className="w-full px-6 py-4 rounded-2xl border-2 border-gray-50 bg-gray-50 focus:bg-white focus:border-pink-500 outline-none font-bold" value={collabPass} onChange={e => setCollabPass(e.target.value)} />
              ...
              <button type="submit" className="w-full py-5 bg-pink-500 text-white rounded-[30px] font-black shadow-xl shadow-pink-100 text-xs uppercase">Cadastrar Ajudante</button>
            </div>
          </form>
        </div>
      )}
