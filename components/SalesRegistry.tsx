...
import { ShoppingBag, CheckCircle2, Search, Banknote, X, Minus, Plus, ShoppingCart, ChevronRight, ArrowLeft, CreditCard, QrCode, History, RotateCcw, Calendar, Sparkles, ArrowRight, UtensilsCrossed, UserCheck, Share2, User } from 'lucide-react';
...
const SalesRegistry: React.FC<SalesRegistryProps> = ({ state, setState }) => {
  ...
  const handleFinalizeSale = () => {
    ...
    setState(prev => {
      ...
      // Atualiza contador de fidelidade do cliente
      ...
      {isCheckoutOpen && (
        <div className="fixed inset-0 bg-pink-950/40 backdrop-blur-md z-[200] flex justify-end">
          <div className="bg-white w-full max-w-md h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
            <header className="p-8 pb-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
              {/* SELEÇÃO DE CLIENTE PARA FIDELIDADE */}
              <div className="space-y-3">
                 <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 flex items-center gap-1"><User size={10}/> Vincular Cliente (Fidelidade)</label>
                 <select 
                 ...
                 >
                   <option value="">Consumidor Final (Não vinculado)</option>
                   {state.customers.map(c => (
                     <option key={c.id} value={c.id}>
                       {c.name} ({(c.purchaseCount || 0)} compras) {(c.purchaseCount || 0) >= 10 ? '⭐ VIP' : ''}
                     </option>
                   ))}
                 </select>
              </div>
              ...
              <div className="space-y-4">
                 ...
                 {cart.map(item => (
                    <div key={item.product.id} className="flex justify-between items-center bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                      <div className="font-black text-gray-700 text-xs">{item.quantity}x {item.product.name}</div>
                      <div className="font-black text-pink-500 text-xs">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.product.price * item.quantity)}</div>
                    </div>
                 ))}
              </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-2">
                    {['PIX', 'Dinheiro', 'Cartão', 'iFood'].map(m => (
                      <button key={m} onClick={() => setPaymentMethod(m as any)} className={`p-4 rounded-2xl border-2 font-black text-[10px] uppercase tracking-widest transition-all ${paymentMethod === m ? 'border-pink-500 bg-pink-50 text-pink-600 shadow-sm' : 'border-gray-50 text-gray-400'}`}>{m}</button>
                    ))}
                 </div>
              </div>
            </div>
            ...
            <div className="p-8 bg-gray-50/80 border-t border-gray-100">
              <div className="flex justify-between items-center mb-6 px-1">
                ...
                <span className="text-3xl font-black text-pink-600 tracking-tighter">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalCart)}</span>
              </div>
              <button onClick={handleFinalizeSale} className="w-full py-5 bg-emerald-500 text-white rounded-[30px] font-black text-lg shadow-xl shadow-emerald-100 hover:bg-emerald-600 active:scale-95 transition-all flex items-center justify-center gap-3">Confirmar e Lançar <ArrowRight size={22} /></button>
            </div>
          </div>
        </div>
      )}
